...
app.post('/search', async (req, res) => {
    ...
    try {
        ...
        const query = `SELECT reviews_title, similarity_cosine(embedding,  [${queryVector}]) as similarity FROM ${CASSANDRA_KEYSPACE}.${CASSANDRA_TABLE} ORDER BY embedding ANN OF [${queryVector}] LIMIT 3`;

        const result = await client.execute(query);

        if (result.rowLength > 0) {
            // map through all rows to construct a response array of the closest product matches based on the review.
            console.log('First result row:', result.rows[0]);
            const matches = result.rows.map(row => ({
                review: row['reviews_title'],
            ...
            res.status(200).send({ message: 'No products with similar reviews found.' });
        }
